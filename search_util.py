# Starter code from CS221 reconstruct assignment
...
class UniformCostSearch(SearchAlgorithm):
    ...
    def solve(self, problem):
        ...
        while True:
            # Remove the state from the queue with the HIGHEST pastCost
            # (priority).
            # Note that for this project higher costs mean better scores.
            state, pastCost = frontier.removeMax()
            if state == None: break
...
class PriorityQueue:
    ...
    # Returns (state with maxiumum priority, priority)
    # or (None, None) if the priority queue is empty.
    def removeMax(self):
        while len(self.heap) > 0:
            priority, state = heapq._heappop_max(self.heap)
